...
# from .predict_det import DetPredictor
...
def main(config,mode):
    ...
    video_outputs = []
    if mode=='signal-img':
        img=cv2.imread(config["Global"]["infer_imgs"])
        if img is None:
            print('读取失败')
            return
        ...
        print(333333333333)
        print(output)
        result = draw_bbox_results(image=img,results=output,input_path=config['Global']['infer_imgs'],save_dir='deploy/images/predict_signal_img/')
        return result  # img numpy数组
    else:
...
if __name__ == "__main__":
    ...
    result = main(config,args.mode)
    name = os.path.basename(config['Global']['infer_imgs'])
