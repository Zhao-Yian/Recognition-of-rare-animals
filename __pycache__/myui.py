...
class Ui_MainWindow(QtWidgets.QWidget):
    def __init__(self, parent=None):
        ...
        self.waitTime = 30  # 帧率 = 1000/self.waitTime
    ...
    def set_ui(self):
        ...
        # self.__layout_data_show2 = QtWidgets.QLabel('FPS')
        self.button_open_camera.setMinimumHeight(50)  # 设置按键大小
        self.button_close.setMinimumHeight(50)

        self.button_close.move(10, 100)  # 移动按键
        self.labelFPS = QtWidgets.QLabel()
        '''信息显示'''
        self.label_show_camera = QtWidgets.QLabel()  # 定义显示视频的Label
        self.label_show_camera.setFixedSize(641, 481)  # 给显示视频的Label设置大小为641x481

        '''把按键加入到按键布局中'''
        self.__layout_fun_button.addWidget(self.button_open_camera,0,Qt.AlignHCenter|Qt.AlignLeft)  # 把打开摄像头的按键放到按键布局中
        self.__layout_fun_button.addWidget(self.button_close,0,Qt.AlignHCenter|Qt.AlignLeft)  # 把退出程序的按键放到按键布局中
        self.__layout_fun_button.addWidget(self.labelFPS,0,Qt.AlignLeft|Qt.AlignBottom)
        '''把某些控件加入到总布局中'''
    ...
    def button_open_camera_clicked(self):
        if self.timer_camera.isActive() == False:  # 若定时器未启动
            ...
            if flag == False:  # flag表示open()成不成功
                ...
                self.timer_camera.start(self.waitTime)  # 定时器开始计时30ms，结果是每过30ms从摄像头中取一帧显示
                self.labelFPS.setText("FPS:{:.2f}".format(1000/self.waitTime))
